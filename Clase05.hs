sumaDivisoresHasta :: Int -> Int -> Int -- Calculate the sum of the divisors of n (integer number) until k(integer number). Ex: If i choose n=6 and k=12, the function returns 12, 1+2+3+6 hwich are the divisors of 6 between 1 and 12 where the remainder is 0
...
-- An integer p > 1 is prime if there is no natural k so 1 < k < p and k divides p/Un entero p > 1 es primo sii no existe un natural k tal que 1 < k < p y k divide a p.
